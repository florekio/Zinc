 ...
 */
export function zinc_eval(source: string): string;

/**
 * Evaluate JavaScript and return both the result and any console output.
 * Returns a JSON string: {"result": "...", "output": ["line1", "line2", ...]}
 */
export function zinc_eval_with_output(source: string): string;

export type InitInput = RequestInfo | URL | Response | BufferSource | WebAssembly.Module;

export interface InitOutput {
    readonly memory: WebAssembly.Memory;
    readonly zinc_eval: (a: number, b: number) => [number, number];
    readonly zinc_eval_with_output: (a: number, b: number) => [number, number];
